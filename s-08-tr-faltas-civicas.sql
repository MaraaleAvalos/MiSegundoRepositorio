...
create or replace trigger estado_inconveniente_trigger
	after insert of falta_civica_id, es_edo_inconveniente on falta_civica
	for each row
	...
	begin
		...
		if v_es_edo_inconveniente = 1 then
			...
			v_num_alcoholimetro := dbms_random.value(1111,9999999);
			v_nivel_alcohol := round(dbms_random.value(0,5));
...
create or replace trigger exceso_vel_trigger
	after insert of falta_civica_id, es_exceso_velocidad on falta_civica
	for each row
	
	declare
	v_falta_id number(10,0);
	v_es_exceso_vel number(1,0);
	v_latitud varchar2(40);
	v_longitud varchar2(40);
	...
	begin
		...
		if v_es_exceso_vel = 1 then
			v_latitud := dbms_random.value(1,50) || '° '||dbms_random.value(1,50)||'\''||orientaciones(round(dbms_random.value(1,4)));
			v_longitud := dbms_random.value(1,50) || '° '||dbms_random.value(1,50)||'\''||orientaciones(round(dbms_random.value(1,4)));
			v_clave_sensor := dbms_random.string('X',8);
			v_vel_max := 80;
			v_vel_reportada := dbms_random.value(80,280);
			dbms_output.put_line('insertando en exceso_velocidad, falta_civica_id: ' || v_falta_id;
			-- inserta en el histórico
			insert into exceso_velocidad
				(falta_civica_id,latitud,longitud,clave_sensor,vel_max_km/h,vel_reportada)
...
create or replace trigger pasarse_alto_trigger
	after insert of falta_civica_id, es_pasarse_alto on falta_civica
	for each row
	...
	begin
		...
		if v_es_pasarse_alto = 1 then
			...
			insert into pasarse_alto
				(falta_civica_id,latitud,nombre_crucero,folio_dispositivo)
